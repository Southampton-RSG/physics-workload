...
class StaffEdit(Page):
    ...
    form = StaffForm.edit(
        ...
        fields=dict(
            load_external=dict(group="row25"),
...
class StaffDetail(Page):
    ...
    header = Header(lambda staff, **_: staff.get_instance_header())
    text = html.p(
        "This staff member has not yet logged into this platform. Their staff account will be linked to their ActiveDirectory ID when they do.",
...
class StaffList(Page):
    ...
    list = StaffTable(
        h_tag=None,
        rows=StaffTable.annotate_rows(Staff.objects),
    )
